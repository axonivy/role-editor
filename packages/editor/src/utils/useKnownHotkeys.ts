...
export const useKnownHotkeys = () => {
  ...
  const addRole = useMemo<KnownHotkey>(() => {
    const hotkey = 'A';
    return { hotkey, label: t('hotkey.addRole', { hotkey: hotkeyText(hotkey) }) };
  }, [t]);

  const deleteRole = useMemo<KnownHotkey>(() => {
    const hotkey = 'Delete';
    return { hotkey, label: t('hotkey.deleteRole', { hotkey: hotkeyText(hotkey) }) };
  }, [t]);
  ...
  return { undo, redo, openHelp, addRole, deleteRole, focusToolbar, focusMain, focusInscription };
};
