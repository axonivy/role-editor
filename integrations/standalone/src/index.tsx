...
export async function start() {
  ...
  const reconnect = async (connection: Connection, oldClient: RoleClientJsonRpc) => {
    await oldClient.stop();
    return initialize(connection);
  };

  await webSocketConnection<RoleClientJsonRpc>(RoleClientJsonRpc.webSocketUrl(server)).listen({
